...
const { log } = require('../blobLogger'); // adjust if this file lives under src/ — see note below
...
async function pushData() {
  ...
  log('INFO', 'upload_to_cosmos.js', 'pushData', 'Starting Cosmos upload run');
    ...
    for (const category of categories) {
      ...
      for (const product of products) {
        try {
          ...
          log('ERROR', 'upload_to_cosmos.js', 'pushData', `Failed: ${product.url} — ${err.message}`);
          totalFailed++;
        }
      }
    }
  ...
  log('INFO', 'upload_to_cosmos.js', 'pushData', `Done — Pushed: ${totalPushed}, Failed: ${totalFailed}`);
}

pushData().catch(err => {
  console.error('Fatal error:', err.message);
  log('ERROR', 'upload_to_cosmos.js', 'pushData', `Fatal error: ${err.message}`);
  process.exit(1);
});
